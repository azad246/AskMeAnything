...
export async function getUserInfo(params:GetUserByIdParams){
    try {
        ...
        return {user,totalQuestions,totalAnswers,badgeCount,reputation:user.reputation}
    } catch (error) {
        console.log(error);
        throw error;
    }
...
export async function getUserQuestions(params:GetUserStatsParams){
    try {
        ...
        const userQuestions=await Question.find({author:userId}).sort({createdAt:-1,views:-1,upvotes:-1})
